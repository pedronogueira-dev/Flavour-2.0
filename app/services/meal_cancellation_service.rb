class MealCancellationService
  ...
  def cancel_meals
    meals = Meal.where("reservation_date <= current_date + 2") # find every meal happening in the space of 2 days with 4 or less people attending
    puts meals.map(&:reservation_date)
    meals.each do |meal|
      if meal.attendees.where.not(status: "Confirmed").count >= 3
